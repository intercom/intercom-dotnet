...
namespace Library.Core
{
	public class Client
	{
		...
		protected virtual IRestRequest BuildRequest (Method httpMethod = Method.GET,
		                                             ...
		                                             String resource = null)
		{
			String final = String.IsNullOrEmpty (resource) ? resrc : resrc + Path.AltDirectorySeparatorChar + resource;
			RestRequest request = new RestRequest (final, httpMethod);
		...
		private ClientResponse<T> HandleNormalResponse<T> (IRestResponse response)
			where T : class
		{
